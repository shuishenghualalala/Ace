...
runtime 启动 → 读 ACE_SECURITY_RUNTIME_TOKEN（≥32 字节）
            → stdout 写 {type:"ready", version:3,
                          capabilities:["deny_read_glob_v1","stdin_once","stream_output",
                                        "duplex_stdio_v1","stdin_bidirectional",
                                        "readonly_roots","full_disk_read"]}
host 每行一个请求：
...
`readonly_roots[]`, `denied_roots[]`, `full_disk_read`, `filesystem_globs[]`, `network_enabled`, `network_rules[]`, `allow_local_binding`,
