...
fn run_with_control(
    ...
    verify_pinned_system_executable(Path::new(SANDBOX_EXEC)).map_err(unavailable)?;
    let policy =
    ...
    // The interactive control channel owns the pipe only when the duplex
    // stream did not claim it above; the two never arrive together.
...
fn create_private_home() -> Result<PathBuf, String> {
    ...
    // The projected HOME can hold credentials; refuse anything that did not
    // land as a real private directory at the requested path.
    let canonical = home
        .canonicalize()
        .map_err(|error| format!("cannot resolve private sandbox home: {error}"))?;
    if canonical != home
        || fs::symlink_metadata(&canonical)
            .map(|metadata| metadata.mode() & 0o777 != 0o700)
            .unwrap_or(true)
    {
        let _ = fs::remove_dir_all(&home);
        return Err("private sandbox HOME resolved to an unsafe path".to_string());
    }
    Ok(canonical)
}
