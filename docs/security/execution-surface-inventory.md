...
| `crew/agent/external/codex_adapter.py` | broker | Codex app-server 双向 stdio 在配置开关开启时禁止宿主回退；关闭后允许兼容直启。 |
| `crew/agent/external/cli_adapter.py` | broker | CLI 对话调用 `security.launch.execute_captured`；配置开关开启时 managed 失败不回退宿主，关闭后才按旧 runtime 直启。模型探测只运行已登记 candidate 的固定 argv，属于认证后的宿主控制面发现，不接收模型 argv。 |
