...
class CrewApp:
    ...
    def _close_persistent_stores(self) -> None:
        ...
        """
        # 全局 process_registry 的 lifecycle 回调绑定在本 App 实例上；只关连接
        # 不解除绑定的话，后续任何 spawn 都会摸到已关闭的 store
        # （sqlite3.ProgrammingError）。完整 shutdown() 已自行重置，此处兜底
        # 走同步关闭路径（测试 autouse / 网关重启）的 App。
        from crew.tools.process_registry import process_registry
