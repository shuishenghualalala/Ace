...
def _security_check_network(args: Any, ctx: CliContext) -> CliResult:
    ...
    result = app.security_service.authorize_network_action(
        sec_ctx,
        action,
        tool_name="security_check",
        public_target=True,
    ...
    return CliResult(
        data={"allowed": result.allowed, "request": result.request, "target": str(target)},
        text="allow" if result.allowed else "ask/deny",
    )
