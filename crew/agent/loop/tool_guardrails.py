...
_BROWSER_USE_IDEMPOTENT_ACTIONS = frozenset(
    {"snapshot", "get_images", "vision", "tab_list", "dialog_status"}
)
...
class ToolCallGuardrailController:
    ...
    def _is_idempotent(self, tool_name: str, args: Mapping[str, Any]) -> bool:
        if tool_name == "browser_use":
            ...
            return action in _BROWSER_USE_IDEMPOTENT_ACTIONS or (
                action == "console" and args.get("clear") is not True
            )
    ...
    def controlled_halt_response(decision: ToolGuardrailDecision) -> str:
        """生成 guardrail halt 后的模型引导文本（采用 _toolguard_controlled_halt_response）。

        当工具防循环触发 hard stop 后，将这段文本作为 assistant 消息写入对话历史，
        让模型知道自己被 halt 了，下一轮可以换策略而不是继续犯同样的错误。
        """
