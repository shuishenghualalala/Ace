...
import { groupByTags, summaryOf } from "../lib/wikiTree";
...
export default function WikiTagView({
  pages,
  selectedId,
  selectedIds,
  highlightedIds,
  ...
  const groups = useMemo(() => groupByTags(pages), [pages]);
  ...
  return (
    <div className="wiki-grouped">
      {groups.map((group) => (
        <section key={group.tag} className="wiki-grouped__section">
          <h4 className="wiki-grouped__title">
            <span className="wiki-card__tag">{group.tag}</span>
  ...
  return (
    ...
    >
      ...
      <div className="wiki-list-item__actions">
        <button className="wiki-tree__act" onClick={onEdit} type="button" title="编辑">
          编辑
        </button>
        <button
