...
class TestE2ETerminalHandler:
    ...
    def _host_launch(self, tmp_path):
        """合并后（ours）宿主路径也必须有未沙箱 ProcessLaunch 才能执行；
        需审批命令在到达 launch 检查前就返回 approval_required。"""
        import os
        ...
        launch = issue_process_launch(
            SecurityContext(
                os_user="os-tgv",
                owner_account_id="owner-tgv",
                workspace_id="workspace-tgv",
                workspace_root=tmp_path,
                session_id="session-tgv",
                request_id="request-tgv",
                task_id="task-tgv",
                cwd=tmp_path,
            ),
