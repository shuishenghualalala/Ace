...
def test_security_check_file_and_network(capsys, cli_home):
    ...
    # 工作区外的路径按本分支语义直接 deny（无法映射为安全的权限根），
    # 判定命令的目标要落在已认证工作区内才有 allow/require_approval 语义。
    workspace_target = task_workspace_path("default", owner_account_id="cli-test") / "ace-cli-check-target"
    code, out = run_cli(
        capsys,
        "security",
        "check-file",
    ...
    assert code == 0
    data = json_out(out)
    # 本分支语义：公网目标默认 require_approval（不再默认放行），
    # 判定命令应返回待审批请求而非直接 allow。
    assert data is not None and data["reason"]
...
def test_security_sandbox_run(capsys, cli_home):
    ...
    helper = packaged_runtime_argv()[0]
    if (
        os.environ.get("ACE_TEST_PACKAGED_RUNTIME_STUB")  # conftest 的哑 stub
        or not Path(helper).is_file()
        or runtime_source_stale(helper)
    ):
        pytest.skip("native security runtime 未随包安装或与源码不一致")
