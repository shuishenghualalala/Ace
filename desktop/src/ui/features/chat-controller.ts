...
import { mergeTeamInternalMessage } from './history-mapping';
...
function sigTeamInternal(msg: ChatMessage, isStreaming: boolean): string {
  ...
  return `team|${msg.id}|${msg.content}|${msg.thinking || ''}|${tools}|${artifacts}|${msg.agentId || ''}|${msg.agentName || ''}|${msg.agentRole || ''}|${msg.agentTone || 0}|${msg.eventType || ''}|${msg.nodeId || ''}|${msg.displayMode || ''}|${msg.collapsedTitle || ''}|${msg.processText || ''}|${durationBucket}|${isStreaming ? '1' : '0'}`;
}
...
export async function sendMessage(text: string): Promise<void> {
  ...
  clearScenarioChip();
  if (isDraftSession(sessionId)) {
    commitDraftSession(sessionId, '新会话', previewText.slice(0, 48), openSessionFn);
  } else {
