...
// ponytail: 惰性初始化——模块级求值 document 会让 node 环境的传递导入（测试）直接崩。
let panelRoot: ParentNode | null = null;
let socketSession = '';
...
function panelQuery<T extends Element>(selector: string): T | null {
  return (panelRoot ?? document).querySelector<T>(selector);
}

function panelQueryAll<T extends Element>(selector: string): NodeListOf<T> {
  return (panelRoot ?? document).querySelectorAll<T>(selector);
}
